...
class IPGeolocationProvider:
    ...
    async def get_location(self, ip: str) -> Dict[str, Any]:
        ...
        try:
            response = await http_client.get(f"https://ipapi.co/{ip}/json/")
        ...
        try:
            response = await http_client.get(f"http://ip-api.com/json/{ip}")
            if response.status_code == 200:
                data = response.json()
    ...
    async def get_my_ip(self) -> Dict[str, Any]:
        ...
        try:
            # Get IP first
            ip_response = await http_client.get("https://api.ipify.org?format=json")
