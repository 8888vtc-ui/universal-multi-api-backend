...
class DogAPIProvider:
    ...
    async def get_random_image(self) -> Dict[str, Any]:
        ...
        response = await http_client.get(f"{self.base_url}/breeds/image/random")
    ...
    async def get_random_images(self, count: int = 5) -> Dict[str, Any]:
        ...
        response = await http_client.get(f"{self.base_url}/breeds/image/random/{count}")
        response.raise_for_status()
        data = response.json()
        return {
    ...
    async def get_breeds(self) -> List[str]:
        """Get all dog breeds"""
        response = await http_client.get(f"{self.base_url}/breeds/list/all")
        response.raise_for_status()
        data = response.json()
    ...
    async def get_breed_images(self, breed: str, count: int = 5) -> Dict[str, Any]:
        ...
        response = await http_client.get(f"{self.base_url}/breed/{breed}/images/random/{count}")
        response.raise_for_status()
        data = response.json()
        return {
            "breed": breed,
...
class CatAPIProvider:
    ...
    async def get_random_image(self) -> Dict[str, Any]:
        ...
        response = await http_client.get(f"{self.base_url}/images/search")
        response.raise_for_status()
        data = response.json()
        if data:
            return {
