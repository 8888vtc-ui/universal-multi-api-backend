...
async def test_endpoint(client: httpx.AsyncClient, method: str, path: str, name: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code in [200, 201, 202]:
            ...
            return False, f"⚠️  {name} - Service Unavailable (API key may be missing)"
        else:
            return False, f"❌ {name} - Status: {response.status_code}, Error: {response.text[:100]}"
...
async def main():
    ...
    if failed == 0:
        ...
        print(f"   ⚠️  {failed} test(s) ont échoué (peut être dû à des clés API manquantes)")
    print("=" * 70)
